...
app.post("/newOrder", isLoggedIn, async (req, res) => {
  ...
  try {
    // Log the order regardless of type
    ...
    if (mode === "BUY") {
      ...
      if (existingHolding) {
        // Recalculate average cost and update quantity
        const totalCost =
      ...
      return res.json({ message: "Buy order placed successfully!" });
    }
    ...
    if (mode === "SELL") {
      ...
      if (existingHolding.qty < qty) {
        return res.status(400).json({
          message: `Insufficient shares. You own ${existingHolding.qty} shares.`,
        });
      }
    ...
    res.status(400).json({ message: "Invalid order mode. Use BUY or SELL." });
  } catch (err) {
    console.error(err);
